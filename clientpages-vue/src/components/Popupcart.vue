<template>
  <div class="box">
    <span v-if="!hasProduct()">Your cart don't have anything :/</span>
    <div v-for="(product, index) in getProductsInCart"  :key="index" class="box-item">
      <img :src="product.image" alt="" class="item-thumb">
      <h3 class="item-name">{{ product.name }}</h3>
      <span class="item-amount">Nums: 1</span>
      <span class="item-price"> {{ product.price }} VND</span>
    </div>
    <div class="cart-info" v-if="hasProduct()">
      <span>Sum: {{ totalPrice() }} VND</span>
